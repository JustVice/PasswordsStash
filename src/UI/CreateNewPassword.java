...
public class CreateNewPassword extends javax.swing.JFrame {
    ...
    private void createNewPassword() {
        if (verify()) {
            ...
            Passwordv2 new_password = new Passwordv2(Base64Encode.Encode_Base64(servicetxt.getText().toUpperCase()), Base64Encode.Encode_Base64(usertxt.getText()), Base64Encode.Encode_Base64(mailtxt.getText()), Base64Encode.Encode_Base64(passwordtxt.getText()), Base64Encode.Encode_Base64(notesPanel.getText()), fav);
            Memory.data.getUserData().getPasswordsList().add(new_password);
            Memory.data.updateInfo();
            Memory.tasks.message("Password " + servicetxt.getText() + " recorded", "Recorded", 1);
            cleanFields();
        } else {
            Memory.tasks.message("YOU HAVE TO PROVIDE AT LEAST THE NAME OF THE SERVICE, PASSWORD AND EITHER USER OR EMAIL INFORMATION TO CREATE A NEW PASSWORD INFO", "Missing info", 2);
        }
    }
    private void helpButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_helpButtonActionPerformed
        Memory.tasks.message(
