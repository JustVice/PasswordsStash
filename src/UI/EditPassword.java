        ...
        if (tempPassword != null) {
            for (int i = 0; i < Memory.data.getUserData().getPasswordsList().size(); i++) {
                if (Memory.data.getUserData().getPasswordsList().get(i).getID() == tempPassword.getID()) {
                    int option = Memory.tasks.askMessage("Are you sure that you want to update this " + Memory.data.getUserData().getPasswordsList().get(i).getService(Memory.return_encoded) + " info?\nThis action cannot be undone.", "Change password info?", 0, 3);
                    if (option == 0) {
                        Passwordv2 passwordUpdate = new Passwordv2(Base64Encode.Encode_Base64(servicetxt.getText().toUpperCase()), Base64Encode.Encode_Base64(usertxt.getText()), Base64Encode.Encode_Base64(mailtxt.getText()), Base64Encode.Encode_Base64(passwordtxt.getText()), Base64Encode.Encode_Base64(notesTxt.getText()), giveFavOrNot());
